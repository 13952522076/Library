...
<title>${message("shop.register.title")}[#if systemShowPowered] - 小书僮™智慧幼教管理平台[/#if]</title>
...
$().ready(function() {
	...
	$captchaImage.click(function() {
		$captchaImage.attr("src", "${base}/common/captcha.ct?captchaId=${captchaId}&timestamp=" + (new Date()).valueOf());
	});
	...
	$registerForm.validate({
		rules: {
			username: {
				...
				remote: {
					url: "${base}/register/check_username.ct",
					cache: false
				}
			},
			password: {
			...
			email: {
				...
				[#if !setting.isDuplicateEmail]
					,remote: {
						url: "${base}/register/check_email.ct",
						cache: false
					}
				[/#if]
			},
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm" action="${base}/register/submit.ct" method="post">
						<table>
							<tr>
								<th>
							...
							[#if setting.captchaTypes?? && setting.captchaTypes?seq_contains("memberRegister")]
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="captcha" name="captcha" class="text captcha" maxlength="4" autocomplete="off" /><img id="captchaImage" class="captchaImage" src="${base}/common/captcha.ct?captchaId=${captchaId}" title="${message("shop.captcha.imageTitle")}" />
										</span>
									</td>
								</tr>
							[/#if]
						...
						<div class="login">
							...
							<dl>
								...
								<dd>
									...
									<a href="${base}/login.ct">${message("shop.register.login")}</a>
								</dd>
							</dl>
						</div>
					</form>
