...
<title>${message("shop.order.info")}[#if systemShowPowered] - 小书僮™智慧幼教管理平台[/#if]</title>
...
$().ready(function() {
		...
		$.ajax({
			...
			success: function(message) {
				if (message.type == "success") {
					location.href = "payment.ct?sn=" + message.content;
				} else {
					$.message(message);
					setTimeout(function() {
						location.reload(true);
	...
	$receiverForm.validate({
		...
		submitHandler: function(form) {
			$.ajax({
				url: "${base}/member/order/save_receiver.ct",
				type: "POST",
...
<body>
	...
	<div class="container order">
		<div class="span24">
			...
			<div class="info">
				<form id="receiverForm" action="save_receiver.ct" method="post">
				...
				<form id="orderForm" action="create.ct" method="post">
					...
					<div class="span24">
						<div class="bottom">
							<a href="${base}/cart/list.ct" class="back">${message("shop.order.back")}</a>
