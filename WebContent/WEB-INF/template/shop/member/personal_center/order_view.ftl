...
$().ready(function() {
	...
	$deliveryQuery.click(function() {
		var $this = $(this);
		$.ajax({
			url: "delivery_query.ct?sn=" + $this.attr("sn"),
			type: "GET",
			dataType: "json",
			cache: true,
			beforeSend: function() {
				...
				<div class="top">
					...
					<span class="handle">
						[#if !order.expired && (order.orderStatus == "unconfirmed" || order.orderStatus == "confirmed") && (order.paymentStatus == "unpaid" || order.paymentStatus == "partialPayment")]
							<a href="payment.ct?sn=${order.sn}" class="button">${message("shop.member.order.payment")}</a>
						[/#if]
						...
						<tr>
							...
							<td>
								[#if orderItem.product??]
									<a href="${base}${orderItem.product.path}?brandId=${orderItem.product.brand.id}&productCategoryId=${orderItem.product.productCategory.id}&productId=${orderItem.product.id}" title="${orderItem.fullName}" target="_blank">${abbreviate(orderItem.fullName, 30)}</a>
								[#else]
