...
<title>${message("shop.member.index")}[#if systemShowPowered] - 小书僮™智慧幼教管理平台[/#if]</title>
					...
					<div>
						<ul>
							...
							<li>
								...
								<em>${member.point}</em>
								<a href="coupon_code/exchange.ct">${message("shop.member.index.exchange")}</a>
							</li>
						</ul>
						<ul>
							<li>
								<a href="order/list.ct">${message("shop.member.index.waitingPaymentOrderCount")}(<em>${waitingPaymentOrderCount}</em>)</a>
								<a href="order/list.ct">${message("shop.member.index.waitingShippingOrderCount")}(<em>${waitingShippingOrderCount}</em>)</a>
							</li>
							<li>
								<a href="message/list.ct">${message("shop.member.index.messageCount")}(<em>${messageCount}</em>)</a>
								<a href="coupon_code/list.ct">${message("shop.member.index.couponCodeCount")}(<em>${couponCodeCount}</em>)</a>
							</li>
							<li>
								<a href="favorite/list.ct">${message("shop.member.index.favoriteCount")}(<em>${favoriteCount}</em>)</a>
								<a href="product_notify/list.ct">${message("shop.member.index.productNotifyCount")}(<em>${productNotifyCount}</em>)</a>
							</li>
							<li>
								<a href="review/list.ct">${message("shop.member.index.reviewCount")}(<em>${reviewCount}</em>)</a>
								<a href="consultation/list.ct">${message("shop.member.index.consultationCount")}(<em>${consultationCount}</em>)</a>
							</li>
						</ul>
					</div>
				</div>
					...
					<table class="list">
						...
						[#list newOrders as order]
							<tr[#if !order_has_next] class="last"[/#if]>
								...
								<td>
									<a href="order/view.ct?sn=${order.sn}">[${message("shop.member.handle.view")}]</a>
								</td>
							</tr>
						[/#list]
					</table>
