...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(preschoolEduXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/preschoolEdu/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File preschoolEduXmlFile = new ClassPathResource(CommonAttributes.PRESCHOOLEDU_XML_PATH).getFile();
			Document document = new SAXReader().read(preschoolEduXmlFile);
			Element element = (Element) document.selectSingleNode("/preschoolEdu/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
