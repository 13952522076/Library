...
public class SmsCellUtil
{
    ...
    public static SmsCellUtil getInstance() throws SMSLibException, IOException, InterruptedException
    {
        if (instance == null)
        {
...
//        password = "moco123456";
     ...
     * 发送号码移动,联通,小灵通可以混和提交 3. 如当前账号不支持某网络发送在QueryUserInfo返回中 /root/
     * maxchars_xx 的值为0 4. 如当前账号不支持某网络的发送同时又提交了此网络的号码会报错5.号码错误 5.
     * msg发送内容最大250个字符,不分区中英文. 6. 发送内容大于相应网络的最大字符数(/root/
     * maxchars_xx)时系统拆成多条发送 7. 每条短信后自动加上用户注册的签名:如 [满堂红] 8.
     * 不加sendtime参数或值为“”时短信立即发送,如果要定制发送时间则加sendtime参数格式为 YYYY-MM-DD HH:MM:SS
