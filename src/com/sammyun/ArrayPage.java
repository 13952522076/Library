...
public class ArrayPage
{
    ...
    {
        String description = "共有数据数:" + this.getTotalRows() + "共有页数: " + this.getTotalPages() + "当前页数为:"
                + this.getPage() + " 是否有前一页: " + this.isHasPreviousPage() + " 是否有下一页:" + this.isHasNextPage()
    ...
    public List getObjects(int page)
    {
        if (page == 0)
            ...
            this.setPage(page);
        this.disposePage();
        if (getPage() * pageRecorders < totalRows)
        {// 判断是否为最后一页
            pageEndRow = getPage() * pageRecorders;
